    ...
    public partial class YeniPaketTanimlama : Form
    {
        ...
        private void kayitTablosunaEkle()
        {
            try
            {
                ...
                ConnectionAndStaticTools.ExecuteWithConnection(conn =>
                {
                    MySqlCommand selectCommand = new MySqlCommand("SELECT * FROM dbdanisankayit WHERE DanisanID = @ID", conn);
                    ...
                    ConnectionAndStaticTools.ExecuteWithConnection(con =>
                    {
                        // Veritabanını güncelliyoruz
                        MySqlCommand updateCommand = new MySqlCommand("UPDATE dbdanisankayit SET İslem = @İslem WHERE DanisanID = @ID", con);
